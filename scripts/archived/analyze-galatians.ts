...
async function main() {
  console.log('🚀 Galatians 전체 분석 시작\n')

  // Fetch all Galatians verses
  ...
  if (!verses || verses.length === 0) {
    ...
    console.error('먼저 verses 테이블에 Galatians 구절을 추가해야 합니다')
    process.exit(1)
  }
  ...
  for (let i = 0; i < toAnalyze.length; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      ...
      fs.writeFileSync('galatians_progress.json', JSON.stringify(progressData, null, 2))
    }
  }
