...
2. **고속 배치 시스템**: 20개 구절씩 병렬 처리 (Mac Mini M4 최적화) ⚡
3. **백그라운드 실행**: 터미널 창 없이 앱 내부에서 직접 실행 ⭐
4. **스마트 대기**: JSON 파일 생성 완료 감지 후 10초만 대기
5. **진행도 추적**: 5초마다 `analysis-json/` 폴더 스캔
...
const BATCH_SIZE = 20; // 원하는 크기로 변경
```

**권장 값**:
- **10개**: 안전함, 저사양 (8GB RAM)
- **20개**: 추천, 고성능 (16GB RAM) ⭐
- **30개**: 공격적, 최고사양 (32GB+ RAM)
- **40개+**: 위험, Rate limit 가능성

## 📊 성능

- **메모리 사용**: ~150MB (Electron 앱 기본) + 병렬 프로세스 ~4GB
- **CPU 사용**: 중간 (20개 병렬 처리, M4 최적화)
- **동시 처리**: 배치당 최대 **20개 구절 병렬 처리** ⚡ (기존 대비 2배 속도)
- **터미널**: 0개 (완전 백그라운드 실행) ⭐
- **배치 소요**: 평균 2-3분 (실제 분석 시간 기반)
- **데이터 크기**: 구절 수 데이터 17KB (NIV 원본 4.5MB 대비 99.6% 절감)
- **권장 환경**: Mac Mini M4 16GB RAM (최적화됨)
