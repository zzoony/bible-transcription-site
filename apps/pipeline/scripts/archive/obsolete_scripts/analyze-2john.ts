...
async function main() {
  console.log('2 John 1:2-13 분석 시작\n')

  // Fetch 2 John 1:2-13 verses
  const { data: verses } = await supabase
    .from('verses')
    .select('id, reference, niv_text')
    .ilike('reference', '2 John 1:%')
    .order('id')

  if (!verses || verses.length === 0) {
    console.error('2 John 구절을 찾을 수 없습니다')
    console.error('먼저 verses 테이블에 2 John 구절을 추가해야 합니다')
    process.exit(1)
  }

  // Filter verses 2-13
  const toAnalyze = verses.filter(v => {
    const match = v.reference.match(/2 John 1:(\d+)/)
    if (!match) return false
    const verseNum = parseInt(match[1])
    return verseNum >= 2 && verseNum <= 13
  })

  console.log(`총 ${toAnalyze.length}개 구절 발견 (2 John 1:2-13)`)
  ...
  for (let i = 0; i < remaining.length; i++) {
    ...
    if ((i + 1) % 5 === 0) {
      ...
      fs.writeFileSync('2john_progress.json', JSON.stringify(progressData, null, 2))
    }
  }
