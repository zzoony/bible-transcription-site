...
// Colossians NIV verses
const verses = `Colossians 1:1|Paul, an apostle of Christ Jesus by the will of God, and Timothy our brother,
...
async function insertVerses() {
  ...
  for (const line of lines) {
    ...
    if (!existing) {
      ...
      if (error) {
        console.error(`Error inserting ${ref.trim()}:`, error)
        errors++
      } else {
        inserted++
        console.log(`Inserted: ${ref.trim()}`)
      }
    } else {
      skipped++
    }
  ...
  console.log(`\nSummary: Inserted: ${inserted}, Skipped: ${skipped}, Errors: ${errors}, Total: ${lines.length}`)
}
