...
async function main() {
  console.log('🚀 James 전체 분석 시작\n')

  // Fetch all James verses
  ...
  if (!verses || verses.length === 0) {
    console.error('❌ James 구절을 찾을 수 없습니다')
    console.error('먼저 verses 테이블에 James 구절을 추가해야 합니다')
    process.exit(1)
  }
  ...
  for (let i = 0; i < toAnalyze.length; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      ...
      fs.writeFileSync('james_progress.json', JSON.stringify(progressData, null, 2))
    }
  }
