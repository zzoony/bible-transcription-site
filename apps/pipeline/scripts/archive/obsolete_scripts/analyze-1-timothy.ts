...
async function main() {
  console.log('🚀 1 Timothy 전체 분석 시작\n')

  // Fetch all 1 Timothy verses
  ...
  if (!verses || verses.length === 0) {
    console.error('❌ 1 Timothy 구절을 찾을 수 없습니다')
    console.error('먼저 verses 테이블에 1 Timothy 구절을 추가해야 합니다')
    process.exit(1)
  }
  ...
  console.log(`\n⏱️  예상 소요 시간: ${Math.round(toAnalyze.length * 3 / 60)} 분`)
  ...
  for (let i = 0; i < toAnalyze.length; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      ...
      fs.writeFileSync('1-timothy_progress.json', JSON.stringify(progressData, null, 2))
    }
  }
