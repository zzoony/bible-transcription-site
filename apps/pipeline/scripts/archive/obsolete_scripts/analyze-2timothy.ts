...
// Load .env from pipeline directory (absolute path)
dotenv.config({ path: '/Users/peter/Dev/bible-transcription-site/apps/pipeline/.env' })
dotenv.config({ path: '/Users/peter/Dev/bible-transcription-site/apps/pipeline/.env.local' })
...
async function main() {
  console.log('2 Timothy 1:2-4:22 분석 시작\n')

  // Fetch all 2 Timothy verses
  ...
  if (!verses || verses.length === 0) {
    console.error('2 Timothy 구절을 찾을 수 없습니다')
    console.error('먼저 verses 테이블에 2 Timothy 구절을 추가해야 합니다')
    process.exit(1)
  }

  // Filter verses 1:2-4:22 (exclude 1:1 which is already done)
  ...
  console.log(`총 ${toAnalyze.length}개 구절 발견 (2 Timothy 1:2-4:22)`)

  // Check which verses are already analyzed
  const verseIds = toAnalyze.map(v => v.id)
  ...
  const remaining = toAnalyze.filter(v => !analyzedSet.has(v.id))
  ...
  console.log(`\n예상 소요 시간: ${Math.round(remaining.length * 5 / 60)} 분`)
  console.log('='.repeat(80))

  for (let i = 0; i < remaining.length; i++) {
    const verse = remaining[i]
    const progress = Math.round(((i + 1) / remaining.length) * 100)

    console.log(`\n[${i + 1}/${remaining.length}] ${verse.reference} (${progress}% 완료)`)
    ...
    if ((i + 1) % 10 === 0) {
      ...
      fs.writeFileSync('2timothy_progress.json', JSON.stringify(progressData, null, 2))
    }
  }
  ...
  console.log('최종 통계')
  console.log('='.repeat(80))
  console.log(`  총 구절: ${remaining.length}`)
  ...
  if (successCount === remaining.length) {
    console.log(`\n2 Timothy 83/83 완료!`)
  } else {
    console.log(`\n일부 구절 실패. 재시도 필요`)
  }
