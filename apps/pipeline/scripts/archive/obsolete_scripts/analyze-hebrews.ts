...
async function main() {
  ...
  if (!verses || verses.length === 0) {
    ...
    console.error('먼저 verses 테이블에 Hebrews 구절을 추가해야 합니다')
    process.exit(1)
  }
  ...
  for (let i = 0; i < toAnalyze.length; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      ...
      fs.writeFileSync('hebrews_progress.json', JSON.stringify(progressData, null, 2))
    }
  }
